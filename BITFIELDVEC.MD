# Bitfield Vectors as an Implementation of Petri Nets

> *Bitfield Vectors store all state of a Petri Net in one integer
> (of arbitrary size), and then performs straightforward operations
> on it to determine in O(1) time whether a transition may fire.*
...
in everyday use.  The technique was pioneered for Erlang, but can
...
## Markings in Bitfield Vectors

We coined the term Bitfielf Vector for the habit of encoding a
...
We currently use the same number of bits for each place; the
images shown below suggest 4 bits for a range of 0 to 15 tokens.
On to pof each of these bitfields, we add a guard bit valued
...
The number of bits per place is determined at compile-time;
...
3*64 upward, and the compile chooses the smallest of
those that can match.  During run-time, it may be detected
that more bits are required, in which case all the values
...
number of tokens initially assigned to each place.  It
should be evident that an initial marking fits the
bitfield vector format as well as any intermediate
marking.

## Sentinel

The guard bits will help us detect underflow when we take
tokens out from a place, and overflow when we add new tokens
to a place.
...
Note that a sentinel can be layered over a marking to
always yield a zero outcome.

## Transition Map

The transition map is a lookup structure from the identity
of a transition to a triple of values that describe the
transition:
  ...
  * The *transition-sentinel* or *transsentinel* is a
...
Arcs from places to transitions subtract tokens, or
*p2t* arcs for short, can only be part of a firing
...
bit valued 0, as in the markings.  This means that the
mere value of the multiplicities in the subber will not
lead to changes in that bit position.

To prepare for testing if a transition can fire, given
a marking, we subtract the subber from the marking, to
find an intermediate value that we shall nickname
...
retrieves the underflow bits, or the former guard bits
...
marking:sub value is dropped.

Note that underflow leads to borrowing from the
higher-up bitfield, if it exists, or to a negative
result (prefixing with 1 bits) if this higher-up
bitfield does not exist.  Since the underflow bit
already indicates the need to abolish the outcome,
these extra changes are of no influence.

## Mixing in Inhibitor Arcs

An *inhibitor arc* forbids a transition to take place
when tokens are present in a place.  It is drawn as an
arc with a circle attached to the transition, rather than
an arrow head.
...
Inhibitors being places that should have their bitfield in
the marking set to zero, can be easily checked with a few
...
transsentinel sets the bits representing this place to 1
so the bits must all be zero-valued.

In comparison to the situation before adding inhibitors,
we now use the more elaborate transsentinel and not the
general sentinel with more bits set to 1, but otherwise
the procedure remains as it was.  To make the procedure
completely standard, a transsentinel is always defined
for a transition; but in lieu of inhibitor arcs leading
to it, the transsentinel value will match that of the
general sentinel.
...
arcs and/or an annotated multiplicity on each arc.  When
no arc exists, the multiplicity is zero.

And again, the multiplicity is for the current transition
and an individual place, each of which are separately
represented in a bitfield vector.  The addend consists of
a vector holding the produced number of tokens in the
...
Given that the new marking should be the new one, we have
